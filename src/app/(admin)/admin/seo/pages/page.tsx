  ...
  const groups: EntityGroup[] = [
    ...
    },
    {
      ...
      targets: CATEGORIES.map((category) => ({
        ...
        defaultDescription: `Browse the Hector 1984 ${CATEGORY_LABEL[category].toLowerCase()} range. Full-grain leather, box-only wholesale ordering, trade terms.`,
        gated: true,
      })),
    },
    {
      ...
      targets: (["summer", "winter"] as const).map((season) => ({
        ...
        defaultDescription: `The Hector 1984 ${seasons[season].label.toLowerCase()} collection, available to approved wholesale accounts.`,
        gated: true,
      })),
    },
  ];
  ...
  if (collections.length > 0) {
    groups.push({
      ...
      targets: collections.map((collection) => ({
        ...
        defaultTitle: `${collection.name} — Hector 1984`,
        defaultDescription: `The ${collection.name} collection from Hector 1984, available to approved wholesale accounts.`,
        gated: true,
      })),
    });
  }
